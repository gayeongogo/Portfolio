...
export default function Modal({ selectedCard, onClose }) {
  const cardDetails = {
    "Card 1": {
      ...
      description:
        "외주 프로젝트로 기존 페이지의 디자인과 구성 전반을 변화시키고 반응형 페이지로 리뉴얼을 진행했습니다. 클라이언트의 요구사항을 충족하며 웹 표준을 준수하여 제작하였습니다.",
      feature: [
      ...
      contribution: [
        <>
          <B>Figma</B>를 사용하여 법무법인 브랜드 아이덴티티를 반영한 비주얼
          디자인과 UI/UX 흐름을 설계
        </>,
        <>
          기존의 어두운 색상 테마를 법무법인의 로고와 조화를 이루는 색상으로
          재구성하여 일관된 브랜드 이미지를 강화
        </>,
        <>
          <B>MediaQuery</B>를 활용하여 모든 화면 크기에서 최적화된 반응형 웹
          페이지로 개선
        </>,
        <>
          <B>Grid 레이아웃</B>을 적용하여 카드형 페이지에서 한 개, 두 개 또는
          여러 개의 아이템을 선택적으로 볼 수 있는 뷰 전환 기능 제공
        </>,
        <>
          <B>filter</B>와 <B>includes 함수</B>를 활용한 검색 기능을 구현하여
          사용자가 입력한 키워드를 기반으로 원하는 정보를 빠르게 탐색할 수
          있도록 개선
        </>,
        <>
    ...
    "Card 2": {
      ...
      description:
        "카테고리별로 URL을 모아 관리하고 여러 플랫폼의 실시간 인기 게시물을 한눈에 확인할 수 있는 북마크 안드로이드 앱 개발에 참여했습니다. React Native를 사용하며 모바일 사용자를 위한 인터페이스 구현을 경험할 수 있었습니다.",
      feature: [
      ...
      contribution: [
        <>
          <B>Figma</B>를 사용하여 전체적인 디자인과 카테고리 관리 및 페이지 이동
          흐름을 설계
        </>,
        <>
          글 작성 시 모바일 사용자에게 익숙한 <B>Bottom Sheet</B>를 사용해 화면
          공간을 효율적으로 활용하고 직관적인 사용자 경험을 제공
        </>,
        <>
          여러 입력 시트 간 발생할 수 있는 오류를 줄이기 위해 <B>Context API</B>
          를 사용하여 Prop 전달 체계를 전역적으로 관리
        </>,
        <>
          <B>Axios</B>를 활용한 데이터 처리로 서버와의 통신 기능을 간결하고
          직관적으로 구현
        </>,
        <>
          <B>Stack Navigation</B>과 <B>Tab Navigation</B>을 혼합하여 각 페이지별
          탐색과 섹션 간 이동을 설계
        </>,
        <>
          맞춤형 추천 게시물 제공을 위해 회원가입 시 입력된 정보와 함께
          추가적으로 연령대, 성별, 관심사를 수집하는 단계를 추가
        </>,
        <>
          로그인 및 회원가입 페이지에서 <B>정규 표현식</B>을 사용해 입력된
          데이터의 유효성을 검사하고 오류 메시지를 제공
        </>,
      ],
    ...
    "Card 3": {
      ...
      description:
        "재생에너지 판매자와 구매자가 효율적인 재생에너지 전력 거래를 할 수 있는 REC 거래 중개 플랫폼 프로젝트에 참여했습니다.",
      feature: [
      ...
      contribution: [
        <>
          <B>Tailwind CSS</B>를 사용하여 스타일을 빠르게 수정하고 재사용
          가능하도록 구현
        </>,
        <>
          사용자가 상품의 위치, 등록순, 판매 상태 등을 드롭 다운 메뉴를 통해
          선택하고 조회할 수 있는 <B>필터링 기능</B> 제공
        </>,
        <>
          <B>Axios</B>를 사용해 상품 조회 시 데이터를 서버로부터 간결하고
          직관적으로 요청한 데이터 통신 구현
        </>,
        <>
          <B>사업자등록정보 진위확인 및 상태조회 API</B>를 활용하여 입력된
          사업자등록번호의 유효성을 검증하고 계속사업자, 휴업, 폐업 여부 등의
          상태 확인 로직 구현
        </>,
        <>
          발전소 정보 등록 시 <B>카드 형태의 리스트</B>를 추가하여 사용자가 각
          발전소의 주요 정보를 한눈에 확인하고 쉽게 관리할 수 있도록 구성
        </>,
        <>
          회원가입 화면에서 <B>진행률을 단계별로 표시</B>하여 사용자의 현재
          절차와 남은 절차를 시각적으로 명확히 파악할 수 있도록 구현
        </>,
      ],
    ...
    "Card 4": {
      ...
      contribution: [
        ...
        <>
          회원가입 시 <B>Firebase 인증 기능</B>을 활용해 <B>switch case문</B>
          으로 다양한 오류 사항의 유효성을 검사하여 처리
        </>,
        <>
          모든 필드 입력 후 글 등록 및 업데이트가 가능하도록 <B>useEffect</B>로
          양식 검사 후 완료 버튼 활성화
        </>,
        <>
          로그인 이전에는 데이터를 <B>local storage</B>에 저장하고, 로그인
          후에는 <B>Firebase</B>를 통해 서버에 업로드하여 관리
        </>,
        <>
          사용자가 선택한 이미지를 미리보기 형태로 보여주기 위해{" "}
          <B>FileReader API</B>를 사용해 데이터 URL로 변환 후 화면에 렌더링
        </>,
        <>
          <B>Datepicker</B>를 사용해 사용자가 날짜를 선택할 수 있도록 구현하고,
          선택한 날짜는 ISO 형식으로 변환해 저장
        </>,
        <>
          <B>postcode API</B>를 사용해 우편번호를 검색하여 주소 입력을 보다
          간편하게 이용할 수 있도록 구현
        </>,
        <>
          하트 모양의 아이콘을 클릭해 사용자가 카페에 대한 평점을 입력할 수 있는{" "}
          <B>인터렉티브 UI</B> 구현
        </>,
      ],
    ...
    "Card 5": {
      ...
      contribution: [
        <>
          텍스트 입력 값을 <B>실시간으로 화면에 반영</B>하여 직관적인 사용 경험
          제공
        </>,
        <>
          랜덤 색상 버튼 클릭 시 무작위 색상을 배경으로 설정하는{" "}
          <B>randomRGB 함수</B>를 구현하여 사용자에게 다양성 제공
        </>,
        <>
          사용자가 <B>로컬 파일을 업로드</B> 하거나 <B>URL을 입력</B>하여 배경
          이미지를 설정
        </>,
        <>
          제작자 버튼 클릭 시 <B>prompt</B>를 사용해 제작자 이름을 입력하고
          콘텐츠에 표시할 수 있도록 구현
        </>,
        <>
          <B>html2canvas 라이브러리</B>를 사용하여 현재 편집한 콘텐츠를 간단하게
          캡쳐하고 이미지로 변환하여 저장
        </>,
      ],
    ...
    "Card 6": {
      ...
      contribution: [
        ...
        <>
          <B>Framer-motion</B> 라이브러리를 사용해 화면 사이트 전반에 애니메이션
          효과를 적용하여 부드러운 인터랙션 경험을 제공
        </>,
        <>
          <B>Intersection Observer API</B>를 사용하여 Career과 Project 화면 간
          스크롤에 따라 내비게이션 메뉴의 활성화 상태가 자동으로 변경되도록 구현
        </>,
        <>
          이메일 버튼 클릭 시 <B>navigator.clipboard.writeText</B>를 사용해
          사용자가 클릭 한 번으로 이메일 주소를 손쉽게 복사할 수 있는 기능 구현
        </>,
      ],
  ...
  return (
    <div>
      ...
      {isImgModalOpen && (
        <ImageModal selectedImg={selectedImg} onClose={closeImgModal} />
      )}
    </div>
  );
}
